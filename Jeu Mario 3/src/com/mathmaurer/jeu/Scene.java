...
import com.mathmauer.personnages.Mario;
import com.mathmauer.personnages.Champ;
import com.mathmauer.personnages.Tortue;
import com.mathmaurer.affichage.CompteARebours;
import com.mathmaurer.affichage.Score;
import com.mathmaurer.audio.Audio;
import com.mathmaurer.objects.TuyauRouge;
import com.mathmaurer.objects.Bloc;
...
public class Scene extends JPanel {
    ...
    private int xFond1;
    private int xFond2;
    private int dx; //pour déplacer l'écran horizontalement
    ...
    public Scene() { // Contructeur 

        super(); // appel le constructeur de la superclasse 

        this.xFond1 = -50; // initialisation des attributs de la scène 
        this.xFond2 = 750;
        this.dx = 0; 
        this.xPos = -1;
        this.ySol = 293;
        ...
        tuyauRouge1 = new TuyauRouge(600, 230); // Création de nouveaux objets avec emplacement
        ...
        this.addKeyListener(new Clavier()); // relier la classe clavier à la classe scène
    ...
    public void deplacementFond() {
        ...
        this.xFond1 = this.xFond1 - this.dx; // mise à jour de la position de fond par rapport à la valeur de dx
    ...
    public void paintComponent(Graphics g){
        super.paintComponent(g); //appelle le paintComponent de la classe parente JPanel
        Graphics g2 = (Graphics2D) g;
    
                // boucle for pour détéction de contact avec un objet 
            for(int i = 0; i < this.tabObjets.size(); i++) { // boucle externe parcourt tous les objets stockés dans this.tabObjets
                if(this.mario.proche(this.tabObjets.get(i))) { // vérifie si Mario est proche d'un objet 
                    this.mario.contact(this.tabObjets.get(i)); // SI Mario est proche, appelle la méthode contact() avec objet en paramètre
                }
            ...
            for (int i = 0; i < this.tabChamps.size(); i++) {
                Champ currentChamp = this.tabChamps.get(i);
            
                // champax
                for (int j = 0; j < this.tabChamps.size(); j++) {
                    if (j != i) {
                        Champ otherChamp = this.tabChamps.get(j);
            
                        if (otherChamp.proche(currentChamp)) {
                            otherChamp.contact(currentChamp);
                        }
                    }
                }
                ...
                for (int j = 0; j < this.tabTortues.size(); j++) {
                    if (j != i) {
                        if (this.tabChamps.get(i).proche(this.tabTortues.get(j))) {
                            this.tabChamps.get(i).contact(this.tabTortues.get(j));
                        }
                    }
                }
            }
        ...
        g2.drawImage(this.imgFond1, this.xFond1, 0, null); // Draw the background image 1
        g2.drawImage(this.imgFond2, this.xFond2, 0, null); // Draw the background image 2
       
        // Images des objets du tableau
        for(int i = 0; i < this.tabObjets.size(); i++) { //boucle for
            g2.drawImage(this.tabObjets.get(i).getImgObjet(), this.tabObjets.get(i).getX(), this.tabObjets.get(i).getY(), null);
        }

        
        for(int i = 0; i < this.tabPieces.size(); i++) { //boucle for
        ...
        if(this.mario.isSaut()) {
            g2.drawImage(this.mario.saute(), this.mario.getX(), this.mario.getY(), null); // Si mario saute, on fait appelle à la méthode saute - position x y classique
        }
        

        else {
            g2.drawImage(this.mario.marche("mario", 25), this.mario.getX(), this.mario.getY(), null); // sinon normal
        }

        }        else {
            g2.drawImage(this.mario.meurt(), this.mario.getX(), this.mario.getY(), null); // sinon normal
        }
...
for(int i = 0; i < this.tabTortues.size(); i++){
    if(this.tabTortues.get(i).isVivant() == true){
        ...
        // Check if Mario's image is not null before drawing
        if (this.mario.getImgMario() != null) {

        } else {
            System.out.println("Mario image is null"); // Print a message if Mario's image is null
        }
    
        g2.drawImage(imgChateau1, 10 - this.xPos, 95, null); // Draw the castle
        g2.drawImage(imgDepart, 220 - this.xPos, 234, null); // Draw the departure image
        ...
        if(this.finDePartie() == true) {
            ...
            if(this.partieGagnee() == true) {
                g2.drawString("Vous avez gagné!! Katta!", 120, 180);
            }
            else{
                g2.drawString("Too bad loser!", 120, 180);
            }
        }
